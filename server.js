...
app.post("/login",upload.none(),async(req,res)=>{
  console.log(req.body);
   ...
   if(userArr[0].password === req.body.password){
    let token = jwt.sign({email:req.body.email,password:req.body.password},"navi")
    let dataToSend ={
     firstName: userArr[0].firstName,
     lastName: userArr[0].lastName,
     age: userArr[0].age,
...
let connectedToMDB = async ()=>{
    try{
    await mongoose.connect("mongodb+srv://Navitha:Navitha2324@cluster0.mxvyppw.mongodb.net/BatchMern2503?retryWrites=true&w=majority&appName=Cluster0");
